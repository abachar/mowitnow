...
public class MowingController {
	 ...
	 */
	private Surface createSurface(String row) throws MowingException {

		// Validate line
		Pattern p = Pattern.compile("^([1-9]\\d*) ([1-9]\\d*)$");
		Matcher m = p.matcher(row);
		if (!m.matches()) {
			throw new MowingException("Invalid surface.");
		}
	 ...
	 * @throws MowingException if the row is invalid
	 */
	private void executeInstructions(Mower mower, String row) throws MowingException {

		// Validate line
		Pattern p = Pattern.compile("^[ADG]*$");
		Matcher m = p.matcher(row);
		if (!m.matches()) {
			throw new MowingException("Invalid mower Instructions at line: " + row);
		}
